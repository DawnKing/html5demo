...
var SoftEngine;
(function (SoftEngine) {
    SoftEngine.Mesh.prototype.computeFacesNormals = function () {
    ...
    var ApplyingTexture = (function (_super) {
        __extends(ApplyingTexture, _super);
        function ApplyingTexture(canvas) {
            _super.call(this, canvas);
            this.depthbuffer = new Array(this.workingWidth * this.workingHeight);
        }
        ApplyingTexture.prototype.clear = function () {
            _super.prototype.clear.call(this);
            for (var i = 0; i < this.depthbuffer.length; i++) {
                this.depthbuffer[i] = 10000000;
            }
        };
        ApplyingTexture.prototype.putPixel = function (x, y, z, color) {
        ...
        ApplyingTexture.prototype.drawPoint = function (point, color) {
            if (point.x >= 0 && point.y >= 0 && point.x < this.workingWidth && point.y < this.workingHeight) {
                this.putPixel(point.x, point.y, point.z, color);
            }
        };
        ApplyingTexture.prototype.clamp = function (value, min, max) {
            if (typeof min === "undefined") { min = 0; }
            if (typeof max === "undefined") { max = 1; }
            return Math.max(min, Math.min(value, max));
        };
        ApplyingTexture.prototype.interpolate = function (min, max, gradient) {
            return min + (max - min) * this.clamp(gradient);
        };
        ApplyingTexture.prototype.project = function (vertex, transMat, world) {
        ...
        ApplyingTexture.prototype.computeNDotL = function (vertex, normal, lightPosition) {
        ...
        ApplyingTexture.prototype.processScanLine = function (data, va, vb, vc, vd, color, texture) {
        ...
        ApplyingTexture.prototype.drawTriangle = function (v1, v2, v3, color, texture) {
        ...
        ApplyingTexture.prototype.render = function (camera, meshes) {
        ...
        ApplyingTexture.prototype.CreateMeshesFromJSON = function (jsonObject) {
            var meshes = [];
            var materials = [];
        ...
        return ApplyingTexture;
    })(SoftEngine.Device);
    SoftEngine.ApplyingTexture = ApplyingTexture;
